...
const EXERCISE_CONFIG := {
	"exercise_id": "horizontal_shoulder_adduction",
...
@onready var map3_controller: Map3Controller = $Map3Controller
...
@onready var push_progress_label: Label = $UI/PushProgressLabel
@onready var direction_label: Label = $UI/DirectionLabel
...
func _ready() -> void:
	...
	map3_controller.push_progress_changed.connect(_on_push_progress_changed)
	map3_controller.action_window_changed.connect(_on_action_window_changed)
	map3_controller.hold_visual_progress_changed.connect(_on_hold_visual_progress_changed)
	map3_controller.direction_changed.connect(_on_direction_changed)
	...
	var total_pushes := int(_session_config["repetitions_per_phase"]) * int(_session_config["phase_count"])
	map3_controller.setup_map(total_pushes)
...
func _on_session_state_changed(current: ExerciseSession.State, _previous: ExerciseSession.State) -> void:
	...
	map3_controller.set_session_active(gameplay_active)
	if gameplay_active:
		exercise_session.set_repetition_input_enabled(map3_controller.is_action_window_open())
	elif current == ExerciseSession.State.TRACKING_LOST:
		map3_controller.handle_hold_interrupted()
...
func _on_session_feedback_changed(code: String) -> void:
	...
	if code == "SESSION_READY" and not map3_controller.is_action_window_open():
		display_code = "WALLS_CLOSING"
...
func _on_repetition_accepted(data: Dictionary) -> void:
	map3_controller.handle_valid_repetition(data)
...
func _on_hold_progress_changed(progress: float, elapsed_ms: int, target_ms: int) -> void:
	...
	map3_controller.set_hold_progress(progress)
...
func _on_session_completed(summary: Dictionary) -> void:
	await map3_controller.play_finish_sequence()
	...
	result_label.text = "HOÀN THÀNH MAP 3\n\nLần đẩy hợp lệ: %d\nLần chưa hợp lệ: %d\nSố phase: %d\nỔn định còn lại: %d/3\nĐánh giá: %s\n%s" % [
...
func _on_push_progress_changed(completed: int, total: int) -> void:
	push_progress_label.text = "TƯỜNG ĐÃ ĐẨY: %d/%d" % [completed, total]


func _on_direction_changed(direction_name: String) -> void:
	direction_label.text = "HƯỚNG: %s" % direction_name
...
func _start_new_session_attempt() -> void:
	_session_attempt += 1
	_session_config["session_id"] = "map3-session-%d-%d" % [Time.get_ticks_msec(), _session_attempt]
...
func _unlock_next_difficulty(stability: int) -> bool:
	if stability < 2 or _difficulty_level() >= 3:
		return false
	var previous_highest: int = Global.get_highest_unlocked_level(3)
	Global.unlock_level_for_action(3, _difficulty_level() + 1)
	return Global.get_highest_unlocked_level(3) > previous_highest
...
func _feedback_text(code: String) -> String:
	const TEXTS := {
		...
		"WALLS_CLOSING": "Tường gai đang siết vào. Hãy chuẩn bị dạng tay.",
		"ACTION_ZONE_READY": "Tường đã dừng. Dạng hai tay sang hai bên và giữ 5 giây.",
		"MOVEMENT_STARTED": "Đã nhận chuyển động.",
		"TARGET_REACHED": "Đúng tư thế. Bắt đầu giữ 5 giây.",
		"HOLD_STARTED": "Giữ nguyên tư thế...",
		"HOLD_PROGRESS": "Tiếp tục giữ — sóng năng lượng đang đẩy tường lùi.",
		"HOLD_COMPLETED": "Đã giữ đủ 5 giây!",
		"HOLD_INTERRUPTED": "Tư thế bị gián đoạn. Hãy thử lại chậm rãi.",
		"REP_VALID": "Tốt lắm! Tường đã bị đẩy lùi thành công.",
		...
		"SESSION_COMPLETED": "Hoàn thành Map 3!",
	}
