...
@onready var map4_controller: Map4Controller = $Map4Controller
...
@onready var climb_progress_label: Label = $UI/ClimbProgressLabel
...
func _ready() -> void:
	...
	map4_controller.climb_progress_changed.connect(_on_climb_progress_changed)
	map4_controller.action_window_changed.connect(_on_action_window_changed)
	map4_controller.hold_visual_progress_changed.connect(_on_hold_visual_progress_changed)
	map4_controller.run_phase_changed.connect(_on_run_phase_changed)
	...
	var total_steps := int(_session_config["repetitions_per_phase"]) * int(_session_config["phase_count"])
	map4_controller.setup_map(total_steps)
...
func _on_session_state_changed(current: ExerciseSession.State, _previous: ExerciseSession.State) -> void:
	...
	map4_controller.set_session_active(gameplay_active)
	if gameplay_active:
		exercise_session.set_repetition_input_enabled(map4_controller.is_action_window_open())
		_update_scrolling()
	elif current == ExerciseSession.State.TRACKING_LOST:
		map4_controller.handle_hold_interrupted()
		$World/ScrollingBackground.set_scrolling(false)


func _on_run_phase_changed(_is_running: bool) -> void:
	_update_scrolling()


func _update_scrolling() -> void:
	var should_scroll := exercise_session.state == ExerciseSession.State.ACTIVE \
			and map4_controller.is_running_on_ledge()
	$World/ScrollingBackground.set_scrolling(should_scroll)
...
func _on_session_feedback_changed(code: String) -> void:
	...
	if code == "SESSION_READY" and map4_controller.is_running_on_ledge():
		display_code = "LEDGE_RUNNING"
	elif code == "SESSION_READY" and not map4_controller.is_action_window_open():
		display_code = "APPROACHING_JUMP"
...
func _on_repetition_accepted(data: Dictionary) -> void:
	map4_controller.handle_valid_repetition(data)
...
func _on_hold_progress_changed(progress: float, elapsed_ms: int, target_ms: int) -> void:
	...
	map4_controller.set_hold_progress(progress)
...
func _on_session_completed(summary: Dictionary) -> void:
	await map4_controller.play_finish_sequence()
	...
	result_label.text = "HOÀN THÀNH MAP 4\n\nLần nhảy hợp lệ: %d\nLần chưa hợp lệ: %d\nSố phase: %d\nỔn định còn lại: %d/3\nĐánh giá: %s\n%s" % [
...
func _on_climb_progress_changed(completed: int, total: int) -> void:
	climb_progress_label.text = "BẬC ĐÃ LEO: %d/%d" % [completed, total]
...
func _start_new_session_attempt() -> void:
	_session_attempt += 1
	_session_config["session_id"] = "map4-session-%d-%d" % [Time.get_ticks_msec(), _session_attempt]
...
func _unlock_next_difficulty(stability: int) -> bool:
	if stability < 2 or _difficulty_level() >= 3:
		return false
	var previous_highest: int = Global.get_highest_unlocked_level(4)
	Global.unlock_level_for_action(4, _difficulty_level() + 1)
	return Global.get_highest_unlocked_level(4) > previous_highest
...
func _feedback_text(code: String) -> String:
	const TEXTS := {
		"SESSION_READY": "Sẵn sàng leo vách đá.",
		"LEDGE_RUNNING": "Đang chạy trên bậc đá. Chuẩn bị vùng nhảy.",
		"APPROACHING_JUMP": "Sắp tới vùng nhảy. Hãy chuẩn bị tư thế.",
		"ACTION_ZONE_READY": "Vùng nhảy sẵn sàng. Ôm hai tay sau đầu và giữ 5 giây.",
		"MOVEMENT_STARTED": "Đã nhận chuyển động.",
		"TARGET_REACHED": "Đúng tư thế. Bắt đầu giữ 5 giây.",
		"HOLD_STARTED": "Giữ nguyên tư thế...",
		"HOLD_PROGRESS": "Tiếp tục giữ — sắp nhảy lên bậc tiếp theo.",
		"HOLD_COMPLETED": "Đã giữ đủ 5 giây!",
		"HOLD_INTERRUPTED": "Tư thế bị gián đoạn. Hãy thử lại chậm rãi.",
		"REP_VALID": "Tốt lắm! Đã nhảy lên bậc tiếp theo.",
		...
		"SESSION_COMPLETED": "Hoàn thành Map 4!",
	}
